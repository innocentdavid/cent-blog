...
const truncate = (str, section = 'description') => {
  if (section = 'description') {
    let res = `${str.slice(0, 120)}...`
    return res
  }
}

const openLoading = (req="open") => {
...
const getAuthorById = async (id) => {
  // const authorRef = await db.collection('authors').doc(id).get();
  // const author = authorRef.data();
  return 'author'
}

function parseMd(md = '# content') {
  //ul
  md = md.replace(/^\s*\n\*/gm, '<ul>\n*');
  md = md.replace(/^(\*.+)\s*\n([^\*])/gm, '$1\n</ul>\n\n$2');
...
const deletePost = (slug) => {
  db.collection('posts').doc(slug).delete().then(res => {
    return res
